...
Precisamos ser transparentes sobre uma limitação desta parte antes de qualquer coisa. Rodar o DeepResearch de verdade exige um servidor do modelo de 30B (local via vLLM, ou remoto via OpenRouter) e chaves pagas de Serper, Jina e SandboxFusion — infraestrutura que a equipe não tinha disponível nesta fase da disciplina. Por isso optamos por **não** executar o modelo e, em vez disso, prever o comportamento do agente lendo o código com cuidado. Os "resultados" que aparecem nas tabelas abaixo são, portanto, previsões fundamentadas em análise estática, não saídas reais do agente. Em cada caso, apontamos o trecho de código que sustenta a previsão e deixamos o status como **"Pendente de execução real"**. Antes de entregar a versão final, pretendemos rodar ao menos os casos prioritários e trocar a previsão pelo resultado observado de verdade.
...
Ao todo, os 13 casos cobrem os doze tipos de entrada exigidos pelo enunciado: esperado, ambiguidade, falta de informação, fora de domínio, dado sensível, reformulação, entrada curta, entrada longa, fonte ausente/conflitante, tentativa de fonte inventada, saída estruturada, conteúdo enviesado e indisponibilidade de serviço.

Mesmo sem rodar nada, já dá para apontar o achado mais preocupante só de ler o código: o CT-10. O pipeline de leitura de página em `tool_visit.py` não tem nenhuma camada de sanitização entre o HTML bruto que vem da Jina e o prompt de sumarização. Na prática, isso é uma porta de entrada plausível para *prompt injection* via conteúdo de terceiros — e conecta diretamente com o risco de confabulação que é o centro do recorte da nossa equipe.
...
Separamos a variabilidade esperada em dois tipos, porque nem toda variação é igualmente preocupante.

A primeira é a variação lexical, e essa é esperada e até desejável: com `temperature=0.85` e `top_p=0.95`, duas execuções do mesmo prompt devem gerar textos diferentes na forma, mesmo preservando o mesmo conteúdo factual. Isso é só equivalência semântica com redação diferente — sem problema.

A segunda é mais delicada: variação factual, ou de fonte. Como a busca é feita ao vivo e sem cache (`NLP_WEB_SEARCH_ONLY_CACHE=false`), o próprio conjunto de páginas retornadas no topo pode mudar de uma execução para outra — principalmente no caso de V-01, que trata de um tópico recente. Isso pode levar o agente a citar fontes diferentes, ou até fatos diferentes, para a mesma pergunta. Aqui não estamos falando de estilo, e sim de conteúdo.

Há ainda um terceiro efeito, mais sutil: os retries dentro de `readpage_jina` reduzem progressivamente o tamanho do conteúdo truncado a cada tentativa de sumarização mal sucedida (`truncate_length = int(0.7 * len(content))`, `tool_visit.py`, linhas 202–221). Ou seja, se o serviço de sumarização falhar de forma intermitente, o mesmo prompt pode gerar resumos de qualidade diferente entre execuções — por um motivo de infraestrutura, não de conteúdo da página em si.

Os mecanismos de controle que já existem no código (`MAX_LLM_CALL_PER_RUN`, o limite de 150 minutos, o limite de 110 mil tokens) evitam que o agente entre em loop infinito, mas não reduzem a variabilidade de conteúdo — são grades de segurança operacionais, não determinísticas. E não encontramos, em nenhum lugar do `.env.example` ou do código, um jeito de fixar uma seed de amostragem ou ativar um modo "determinístico" (`temperature=0`). Achamos que vale a pena a equipe do projeto expor isso como opção explícita, para casos em que reprodutibilidade importa mais do que diversidade de resposta — comparação de benchmark é o exemplo óbvio.
